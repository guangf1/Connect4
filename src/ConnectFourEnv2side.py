...
class ConnectFourEnv(gymnasium.Env):
    ...
    def __init__(self, opponent=None, render_mode=None, first_player=None, main_player_name='AI'):
    ...
    def step(self, action, play_opponent=True):
        ...
        reward += self.count_streaks(self.last_move_row, self.last_move_col)

        if action == self.COLUMNS_COUNT // 2:
            reward += 0.0005

        if self.is_threatening_move(action):
            reward += 0.03  
        ...
        if play_opponent and self._opponent is not None:
            ...
            return self.board, reward + (-1 * opponent_result[1]), opponent_result[2], opponent_result[3], opponent_result[4]

        return self.board, reward, False, False, {}
    
    def count_streaks(self, row, col):
        if row is None or col is None:
            return 0

        player = self.board[row, col]
        score = 0
        CHAIN_REWARD = {2: 0.005, 3: 0.01}
        ...
        for dr, dc in directions:
            ...
            for step in range(-3, 4):
                ...
                if 0 <= r < self.ROWS_COUNT and 0 <= c < self.COLUMNS_COUNT and self.board[r, c] == player:
                    ...
                    if count in CHAIN_REWARD:
                        score += CHAIN_REWARD[count]
                    count = 0
        return score
    
    def is_threatening_move(self, action):
        for row in range(self.ROWS_COUNT - 1, -1, -1):
            if self.board[row, action] == 0:
                self.board[row, action] = -1
                is_threat = self.check_win_around_last_move(row, action)
